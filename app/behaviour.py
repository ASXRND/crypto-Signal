...
class Behaviour():
    ...
    async def run(self, market_pairs, output_mode):
        """Run the behaviour logic

        Args:
            market_pairs (list): List of market pairs to analyze.
            output_mode (str): Output mode for the analysis.
        """
        ...
        await self.notifier.notify_all(new_result)  # Добавлено await
        ...
        for exchange in market_data:
            ...
            for market_pair in market_data[exchange]:
                ...
                if output_mode in self.output:
                    ...
                    self.logger.warning(f"Unknown output_mode '{output_mode}' provided to Behaviour._test_strategies. Skipping output for {market_pair}.")
    ...
    def _get_indicator_results(self, exchange, market_pair):
        ...
        for indicator in self.indicator_conf:
            if indicator not in indicator_dispatcher:
                self.logger.warning("No such indicator %s, skipping.", indicator)
                continue
    ...
    def _get_informant_results(self, exchange, market_pair):
        ...
        for informant in self.informant_conf:
            if informant not in informant_dispatcher:
                self.logger.warning("No such informant %s, skipping.", informant)
                continue
    ...
    def _get_crossover_results(self, new_result):
        ...
        for crossover in self.crossover_conf:
            if crossover not in crossover_dispatcher:
                self.logger.warning("No such crossover %s, skipping.", crossover)
                continue
