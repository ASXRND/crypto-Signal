...
class Notifier():
    ...
    async def notify_all(self, new_analysis):
        ...
        await self.notify_telegram(new_analysis)  # Добавлено await
    ...
    async def notify_telegram(self, new_analysis):
        if self.telegram_configured:
            message = self._indicator_message_templater(
                new_analysis,
            ...
            if message.strip():
                ...
                try:
                    await self.telegram_client.notify(message)
                except BadRequest as e:
                    self.logger.error("Failed to send Telegram message: %s", str(e))
                    if "unsupported start tag" in str(e):
                        sanitized_message = message.replace('<', '&lt;').replace('>', '&gt;')
                        await self.telegram_client.notify(sanitized_message)
