...
class Ichimoku(IndicatorUtils):
    def analyze(self, historical_data, signal=['leading_span_a', 'leading_span_b'], hot_thresh=None, cold_thresh=None):
        ...
        for index in range(tenkansen_period, ichimoku_df_size):
            ...
            ichimoku_values.loc[ichimoku_values.index[index], 'tenkansen'] = (tankansen_min + tankansen_max) / 2
        ...
        for index in range(kijunsen_period, ichimoku_df_size):
            ...
            ichimoku_values.loc[ichimoku_values.index[index], 'kijunsen'] = (kijunsen_min + kijunsen_max) / 2
        ...
        for index in range(leading_span_b_period, ichimoku_df_size):
            ...
            ichimoku_values.loc[ichimoku_values.index[index], 'leading_span_b'] = (
        ...
        for index in range(0, ichimoku_df_size):
            span_hot = ichimoku_values['leading_span_a'].iloc[index] > ichimoku_values['leading_span_b'].iloc[index]
            close_hot = dataframe['close'].iloc[index] > ichimoku_values['leading_span_a'].iloc[index]
            if hot_thresh:
                ichimoku_values.loc[ichimoku_values.index[index], 'is_hot'] = span_hot and close_hot

            span_cold = ichimoku_values['leading_span_a'].iloc[index] < ichimoku_values['leading_span_b'].iloc[index]
            close_cold = dataframe['close'].iloc[index] < ichimoku_values['leading_span_a'].iloc[index]
            if cold_thresh:
                ichimoku_values.loc[ichimoku_values.index[index], 'is_cold'] = span_cold and close_cold
